...
pub fn compute_freq_adjustments(
    state: &AppState,
    file_max_freq: f64,
    tile_height: usize,
) -> Option<Vec<f32>> {
...
fn compute_freq_adjustments_inner(
    state: &AppState,
    file_max_freq: f64,
    tile_height: usize,
) -> Option<Vec<f32>> {
    ...
    if show_noise {
        ...
        let show_notch = {
            ...
            if dsp_on {
                ...
                match state.display.filter_notch().get_untracked() {
                    ...
                    DisplayFilterMode::Auto | DisplayFilterMode::Same => {
        ...
        // custom strength when Custom, or playback noise floor when Same/fallback.
        {
            ...
            let (nf_opt, strength) = if dsp_enabled && matches!(nr_mode, DisplayFilterMode::Auto) {
                ...
                (
                    state.noise_reduce.floor().get_untracked(),
                    state.noise_reduce.strength().get_untracked(),
                )
            } else {
                (None, 0.0)
            };
